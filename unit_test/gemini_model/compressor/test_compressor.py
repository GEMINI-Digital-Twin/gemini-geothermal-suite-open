...
compressor: mass_flow=0.01 kg/s, pressure_in=1 bar,
pressure_out=200 bar, gas_constant=4124.2 J/(kg.K), with mechanical
...
COMPRESSOR_INPUT = {
    "mass_flow": 0.01,  # kg/s
    "pressure_in": 1.0,  # bar
    "pressure_out": 200.0,  # bar
    "temperature_in": 24.85,  # degC (298.0 K)
}
...
class TestCompressor(unittest.TestCase):
    ...
    def test_compressor_calculation(self):
        ...
        self.assertEqual(y["pressure_in"], 1.0)
        self.assertEqual(y["pressure_out"], 200.0)
        self.assertEqual(y["temperature_in"], 24.85)

    def test_zero_mass_flow_gives_zero_power(self):
        """Test zero mass flow results in zero compressor power."""
        u = dict(COMPRESSOR_INPUT)
        u["mass_flow"] = 0.0
        self.compressor.calculate_output(u)
        y = self.compressor.get_output()
